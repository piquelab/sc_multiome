...
fn <- paste("../../enloc_analysis/analysis_correct/INTACT_output/", trait, "_intact.txt", sep="")
...
summ <- map_dfr(1:nt, function(i){
   ...
   ### 
   gene1 <- res%>%dplyr::filter(FDR_twas<th0)%>%pull(Gene)%>%unique()
   gene2 <- res%>%dplyr::filter(FDR_intact<th0)%>%pull(Gene)%>%unique() 
   gene3 <- res%>%dplyr::filter(FDR_intact<th0, nannot>1)%>%pull(Gene)%>%unique()

   df <- data.frame(traits=trait, trait_short=trait2,
      ntwas=length(gene1), nINTACT=length(gene2), n_annoted=length(gene3))
   df
})
...
opfn <- "./3_examples_output/1_intact_summ_FDR0.05.xlsx"
