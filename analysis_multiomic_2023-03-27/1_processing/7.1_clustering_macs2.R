...
Idents(combined2) <- "predicted.id.12"

head(x = Idents(object = combined2))

# set a reasonable order for cell types to be displayed when plotting
levels(combined2) <- c("CD4 Naive", "CD4 TCM", "CD4 CTL", "CD4 TEM",
                       "CD4 Proliferating", "CD8 Naive", "dnT",
                       "CD8 TEM", "CD8 TCM", "CD8 Proliferating", "MAIT", "NK",
                       "NK_CD56bright", "NK Proliferating", "gdT",
                       "Treg", "B naive", "B intermediate", "B memory",
                       "Plasmablast", "CD14 Mono", "CD16 Mono",
                       "cDC1", "cDC2", "pDC", "HSPC", "Eryth", "ASDC",
                       "ILC", "Platelet")
...
#write_rds(combined2, file="./7.1_Clustering_macs2.outs/1_seurat.cluster.combined.rds")

write_rds(combined2, file="./7.1_Clustering_macs2.outs/1_seurat.cluster.combined.mitofilt.rds")

combined2 <- read_rds("./7.1_Clustering_macs2.outs/1_seurat.cluster.combined.mitofilt.rds")
...
for (x in c(0.1)) {
      combined2 <- FindClusters(combined2, graph.name = "wsnn", algorithm = 3, resolution = x, verbose = FALSE)
        png(paste("./7.1_Clustering_macs2.outs/UMAP_joint_clustering_mitofilt_", x, ".png", sep=""))
...
png("./7.1_Clustering_macs2.outs/UMAP_joint_labels_mitofilt.png")
DimPlot(combined2, reduction = "umap", group.by = "predicted.id.11", label = TRUE, repel = TRUE) + NoLegend()
dev.off()

png("./7.1_Clustering_macs2.outs/UMAP_joint_labels_12_mitofilt.png")
DimPlot(combined2, reduction = "umap", group.by = "predicted.id.12", label = TRUE, repel = TRUE) + NoLegend()
dev.off()
...
#----------------------------------------------
#----checking annotation with previous file----
#----------------------------------------------
combined2_prev <- read_rds("./3_Clustering.outs/1_seurat.cluster.combined.mitofilt.rds")

colnames(combined2_prev@meta.data)


meta_prev <- combined2_prev@meta.data %>% as.data.frame() %>% dplyr::select(NEW_BARCODE, predicted.id.12, wsnn_res.0.1)
head(meta_prev)


meta <- combined2@meta.data %>% as.data.frame() %>% dplyr::select(NEW_BARCODE, predicted.id.12, wsnn_res.0.1)
head(meta)


meta.join <- left_join(meta_prev, meta, by="NEW_BARCODE")
head(meta.join)

for (i in 1:nrow(meta.join)){
    if(meta.join$wsnn_res.0.1.x!=meta.join$wsnn_res.0.1.y){
        print(i)
        }
    }
