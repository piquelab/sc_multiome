...
head(cgDE.df$Cluster)

MCls <- unique(cgDE.df$MCls)
#MCls
contrast <- unique(cgDE.df$contrast)
#contrast



##----celltype----
cl <- paste(rep(contrast, times=length(unique(cgDE.df$MCls))),
            rep(MCls, each=length(unique(cgDE.df$contrast))), sep=".")
...
head(cgDE2 %>% dplyr::select(!geneID))
...
#sig_cgDE2 <- cgDE2 %>% dplyr::filter(p.adjust<0.1)
#nrow(sig_cgDE2)


sig_cgDE2 <- cgDE2 %>% dplyr::filter(maxGSSize>10, maxGSSize<500, ngene>10, p.adjust<0.1)
head(sig_cgDE2)
nrow(sig_cgDE2)


fig1 <- enrichplot::dotplot(sig_cgDE2, x="ClusterNew", showCategory=5)+
...
###
figfn <- "./4_enrichment_2.outs/Figure1.1_GO_DE_ClusterNew_treat_highres.png"
