...
for (oneMCl in MCls){
    ...
    figs <- lapply(1:nrow(dfcomb), function(i){
        ...
        sel2 <- dfcomb$x2[i]
        ###
        plotDF <- df2[, c("barcode", "MCls", "treat2", sel1, sel2)]
        names(plotDF)[4] <- "x"
        names(plotDF)[5] <- "y"
    ...
    ###
    ###
    figfn <- paste(outdir, "Figure_LDA_", oneMCl, ".scatters.png", sep="")
...
p <- Heatmap(mat2, name="SCC", na_col="grey90", 
    ...
    cell_fun=function(j, i, x, y, width, height, fill){
       grid.text(round(mat2[i,j],digits=3), x, y, gp=gpar(fontsize=10))
    })
