...
outdir <- "./9_RNA.dynamic2_output/Filter0/"
...
### 1. keep spliced genes 
anno <- data.frame(rn=rownames(sc))%>%
   mutate(ensgene=gsub("[SU]-|\\.[0-9]*","",rn), rnz=rowSums(counts), uns=grepl("S-", rn))%>%
   filter(uns, rnz>0) 
   ## mutate(sel=ensgene%in%DEG)%>%
   ## filter(sel,uns)

sc2 <- subset(sc, features=anno$rn)

### output
opfn0 <- paste(outdir, "1_SCAIP.spliced.rds", sep="")
write_rds(sc2, opfn0)
 

### 2.  keeping Batch 1,4,5,6
fn <- paste(outdir, "1_SCAIP.spliced.rds", sep="")
sc2 <- read_rds(fn)
###remain Batch 1, 4 and 5
scNew <- subset(sc2, BATCH%in%c("SCAIP1", "SCAIP4", "SCAIP5", "SCAIP6"))
###
opfn1 <- paste(outdir, "2_Batch1456.spliced.rds", sep="")
...
fn <- paste(outdir, "2_Batch1456.spliced.rds", sep="")
sc2 <- read_rds(fn)
...
for (i in 1:4){
   ...
   if (correct=="Old"){
      ...
      opfn <- paste(outdir, "3_MCl.", oneMCl, ".old.rds", sep="") 
      write_rds(sc0, opfn)
   }else{
      ...
      opfn <- paste(outdir, "3_MCl.", oneMCl, ".harmony.rds", sep="")
      write_rds(sc0, opfn) 
      cat("Harmony Done", "\n")  
   }
...
df2 <- map_dfr(MCls, function(oneMCl){           
   ##
   fn <- paste(outdir, "3_MCl.", oneMCl, ".old.rds", sep="")
   sc0 <- read_rds(fn)
...
fig0 <- ggplot(df2, aes(x=UMAP_1, y=UMAP_2))+      
   ...
   facet_grid(MCls~treat2, scales="free_y",
      labeller=labeller(treat2=c("CTRL"="CTRL",
         "LPS"="LPS", "LPS-DEX"="LPS+DEX", 
         "PHA"="PHA", "PHA-DEX"="PHA+DEX")))+
...
figfn <- paste(outdir, "Figure3.1_umapDensity.pdf", sep="")
