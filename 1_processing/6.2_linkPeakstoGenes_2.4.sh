...
mkdir -p 6.2_linkPeakstoGenes_2.4_slurm
...
cat file_names.txt  |  while read sample; do
echo ${sample}
sbatch -q primary  -N1-1 -n 2 --mem=240G -t 2000 --job-name=${sample} -o "6.2_linkPeakstoGenes_2.4_slurm/slurm_${sample}" --wrap "Rscript 6.2_linkPeakstoGenes_2.4.R $sample"
sleep 0.5
done
