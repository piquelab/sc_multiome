...
fn <-"./1_vars_gene.outs/1_cv.rds"
cvt2 <- read_rds(fn)
###
fn2 <- "./1_vars_gene.outs/1_geneExpr.rds"
...
fn <- "./1_vars_gene.outs/all_genes.vars.txt"
res <- read.table(fn)
names(res) <- c("sampleID", "MCls", "treat2", "Residual", "genes")
##
opfn <- paste(outdir, "all_genes.vars.txt", sep="")
